...
import secretsgen as sec  # noqa: F401 - 供测试直接使用
from audio import CollectSink
from core import RelayClient, ST_REGISTERED, ST_BRIDGED, ST_FATAL
...
def relay_proc(tmp_path_factory):
    ...
    r = subprocess.run(
        ...
        cwd=repo / "relay", capture_output=True, text=True, timeout=120,
    )
    if r.returncode != 0:
        pytest.skip(f"relay 构建失败: {r.stderr}")
