...
手机端采音程序（Kotlin，零第三方依赖）：前台服务 + TLS 内部 TOFU 连接 server，
...
1. 打开 App → 设置（右上角齿轮）：
   - **「扫描局域网服务器」**：同一 Wi-Fi 下自动发现 server，弹列表直接选择
     （选中即保存为「本地服务器」，本地优先连接）；扫不到时手动操作 ↓
   - 「导入配置串」粘贴 server 启动横幅的 `rv://<IP>:9432`（填入**远程服务器地址**）；
     首次连接自动建立内部信任
   - 「添加设备」输入 **Mac 端显示的临时/永久秘密**（新增即激活）
   - （可选）编辑设备名/开关免提常开/回声消除
2. 返回主界面：**App 打开即自动连接**（本地→远程依次尝试，状态条显示当前链路），
   底部大按钮 **按住说话**（松开即停）；状态条显示连接态，
   **点按状态条 = 连接/停止**；多台设备点芯片切换（切换即自动重连）
...
- 秘密仅存本机 SharedPreferences，网络层只传 SHA-256 hex；
  TLS 首次连接使用内部 TOFU，之后按服务器地址固定证书（`FingerprintTrustManager`），不要求用户输入指纹
