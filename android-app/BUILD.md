...
适用代码基线：`app/build.gradle.kts`（v0.2.0）。
...
### 3.7 首次启动配置（协议 v3 流程，2026-08-28 起）

**手机端只需输入 Mac 显示的配对秘密**；不再输入服务器 regkey 或证书指纹。
要连接的每一台 Mac 以「设备」条目保存（名称+秘密，新增即激活）。
首次使用三步：

1. **设置 → 导入配置串**：粘贴 server 启动横幅打印的 `rv://IP:端口`；首次连接
   自动建立内部服务器信任，不需要输入证书指纹
2. **主界面「＋添加」**：输入 Mac 图形客户端显示的临时秘密（或永久秘密），
   设备名可留空——首次连接成功后 Mac 设备名会自动回填
3. 返回主界面后自动连接，点状态条可停止/恢复；中间圆钮**按下即传、松开即停**（按住说话）

辅助项（设置页）：回声消除（Mac 外放时建议开启）映射 `VOICE_COMMUNICATION` 音源；
「免提常开」开启后从按住说话退回持续推流（会议场景）。状态栏颜色：
绿=传输中（含已发送帧数）/ 蓝=连接或等待对端 / 红=错误 / 灰=停止。
设备芯片单击切换即重连；长按管理（改名/换秘密/删除）。
...
对比 server：`go build` 一条命令就完事，因为 Go 语言的设计目标是把运行时和
...
server 的复杂度在**部署环境**（要公网 IP、防火墙；证书指纹由客户端内部 TOFU 管理），构建只需一条命令；
