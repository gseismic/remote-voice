...
### 3.7 首次启动配置（协议 v2 流程，2026-08-26 起）

**手机上不再输入密码**；要连接的每一台 Mac 以「设备」条目保存（名称+秘密，新增即激活）。
首次使用三步：

1. **设置 → 导入配置串**：粘贴 server 启动横幅打印的 `rv://IP:端口?f=指纹`
   （v2 已不含秘密字段），自动填好服务器与指纹
2. **主界面「＋添加」**：输入 Mac 图形客户端显示的临时秘密（或永久秘密），
   设备名可留空——首次连接成功后 Mac 设备名会自动回填
3. 点**「连接」**，听到中间圆钮**按下即传、松开即停**（按住说话）
...
server 的复杂度在**部署环境**（要公网 IP、防火墙、证书指纹），构建只需一条命令；
