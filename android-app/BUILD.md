...
### 3.7 首次启动配置（2026-08-26 起的极简流程）

主界面只需**一个连接密码**（与 relay 的 token.txt、Mac 端 `--token` 三处一致；
可用输入框旁「生成」按钮产生随机强密码）。其余配置在右上角菜单 →「设置」：

1. **服务器地址**：已预置默认 `43.139.226.138:9432`，可改
2. **服务端指纹**：relay 首启打印的 64 位 hex（必填，防中间人）
3. 或直接把 relay 启动横幅打印的 `rv://IP:端口?t=密码&f=指纹` 整行粘贴到
   设置顶部「导入配置串」，一键填全（含主界面密码）

填好后回主界面点「开始推流」。Mac 外放时建议在设置中打开「回声消除」
（映射 `VOICE_COMMUNICATION` 音源）。状态栏按颜色区分：
绿=推流中（含已发送帧数）/ 蓝=连接或等待对端 / 红=错误 / 灰=停止。
